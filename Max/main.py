...
class home(base_app):
    ...
    def serverConnect(self):
        if self.header.CType.get() == "UDP                    ":
            ...
            self.inlet_time = self.lslServer()
        #Close popup
    ...
    def generateWaveletCNN(self):
        script_dir = os.path.dirname(__file__) #<-- absolute dir the script is in
        script_dir = os.path.dirname(script_dir)
        rel_path = "/GUI/UserData/"
        abs_file_path = os.path.join(script_dir, rel_path)
        self.model = CNN(script_dir + rel_path + self.header.user.get() + "/")
        return
        ...
        while self.read == True:
            for i in range(9):
                if i == 8:
                    ...
                    if self.wavelet_on:
                        sample,timestamp = self.inlet_time.pull_sample()
    ...
    def offStateTimer(self):
        ...
        if self.screens[self.SCR_CALIBRATION9].counter_1.get() > 0:
            ...
            #pygame.mixer.music.play(loops=0)
            if self.wavelet_on:
                threading.Thread(target=self.generateWaveletCNN).start()
            else:
    ...
    def readGeneralBrainData(self):
        ...
        data = np.ndarray((np.max(scales),int(fs),8))
        while self.model != None:
            for i in range(8):
                if wavelet_on:
                    sample, timestamp = self.inlet_time.pull_sample()
                    if i not in self.channel_data:
                        coeffs,freq = pywt.cwt(sample,scales,'morl',1/fs)
                        data[:,:,i] = coeffs
                else:
                    sample,timestamp = self.inlet.pull_sample()
                    if i not in self.channel_data:
            ...
            if wavelet_on:
                modelOutput = self.model.predict(data)
            else:
                temp = [self.dataInput]
